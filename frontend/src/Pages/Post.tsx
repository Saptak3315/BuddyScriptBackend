...
import { addpost, appendPost, setpost } from "../redux/authSlice";
...
const [hasMore, setHasMore] = useState(true);
  ...
  // }, [obj.id];
  const LoadMore = () => {
    axios.get(`http://localhost:3333/ap/${page + 1}`, { withCredentials: true })
      .then(res => {
        ...
        if (Array.isArray(newPosts)) {
          ...
          console.error("Expected an array but got:", newPosts);
        }
      })
      .catch(err => {
  ...
  const handleAddPost = (e) => {
    ...
    // };
    axios.post('http://localhost:3333/cp', { userId: currentUser.id, content: newPost }, { withCredentials: true }).then(res => {
          ...
          <div className="col-xl-6 col-lg-6 col-md-12 col-sm-12">
            <div className="_layout_middle_wrap">
              <div className="_layout_middle_inner">
                ...
                {posts?.map((ele: any, index: any) => {

                  // console.log(ele.id)
                  //const currt = Date.now();
                  //const difInMin = Math.floor((currt - ele.timeofcreate) / 1000 / 60);

                  return <PostCard obj={ele} key={ele.id} />;
                })}
                <Button color="red" variant="filled" onClick={LoadMore}>
                  Load more
                </Button>
